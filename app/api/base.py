...
class BaseQuery(object):
    ...
    def _create(self, args):
        ...
        try:
            db.session.commit()
            return True, [r.id for r in result]
...
class Service(BaseParse, BaseQuery, MethodView):
    __model__ = None
    decorators = [view_route, login_required]
    ...
    def get(self, key=None):
        ...
        if key is not None:
            ...
            self._add_more_info(query, data)
    ...
    def post(self):
        ...
        if data:
            _res = self._post_check(data)
            if _res:
                return _res.data

            rs, result = self._create(args=data)
            if rs:
    ...
    def delete(self, key=None):
        ...
        self._delete_check(key)
        if not self._delete(key=key):
            res.update(code=ResponseCode.Fail)
        return res.data

    def _delete_check(self, data):
        pass